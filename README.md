...
2. Deploy the `s3bucket.yaml` file to create an S3 bucket where the nested CloudFormation templates will be stored. Edit the parameter overrides to customize the deployment:  
	...
	--tags Project=aws-to-do-list-two-tier-website
	```
...
5. Create an SSM SecureString parameter for the DB master password:  
    ```bash
	aws ssm put-parameter \
	--name /todoapp/db/master_password \
	--type SecureString \
	--value 'Your_Password' \
	--overwrite
	```

6. Edit the `params.json` and `parent-template.yaml` file to customize the deployment.  
   ...
   - `DBUser` and `DBPasswordParameterName` are the credentials that will be used to access the RDS instance. The DB password is stored in the SSM SecureString parameter `DBPasswordParameterName`.  
...
7. Deploy the CloudFormation stack:  
	...
	--tags Project=aws-to-do-list-two-tier-website
	```
> **Note**: Ensure the AWS CLI user (`aws configure`) or CloudFormation assumed role has sufficient permissions to manage **S3**, **EC2**, **RDS**, **VPCs**, **Subnets**, **Route Tables**, **Security Groups**, and **IAM resources**.  
